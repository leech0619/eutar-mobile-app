...
class _BusRouteDetailScreenState extends State<BusRouteDetailScreen> {
  ...
  void _validateRouteData() {
     if (widget.route['trips'] is! List || (widget.route['trips'] as List).isEmpty) {
  ...
  void _showRouteOnMap() {
    ...
    try {
      setState(() {
        ...
        if (departures is! List || departures.isEmpty || departures[0] is! Map<String, dynamic> || departures[0]['stop'] == null) {
        ...
        final utarStops = allStops
            .where((stop) => stop.contains('UTAR'))
            .toList();
        final housingStops = allStops
            .where((stop) => !stop.contains('UTAR'))
            .toList();
        ...
        if (isDepartingFromUTAR) {
          ...
          utarStops.sort((a, b) =>
              (utarOrderDepart[a] ?? 99).compareTo(utarOrderDepart[b] ?? 99)); // Use default high value for unknowns
          orderedStops = [...utarStops, ...housingStops];
        } else {
          // Sort UTAR stops N -> G -> D (Reverse logic applies here)
           utarStops.sort((a, b) =>
              (utarOrderArrive[a] ?? 99).compareTo(utarOrderArrive[b] ?? 99)); // Use default high value for unknowns
        ...
        for (int i = 0; i < orderedStops.length; i++) {
          ...
          _markers.add(
            Marker(
              markerId: MarkerId(stop + i.toString()), // Add index for potential duplicate names
        ...
        // --- Add Polyline ---
        final List<LatLng> routeCoordinates = orderedStops
            .map<LatLng>((stop) => _getLocationForStop(stop))
            .toList();

        if (routeCoordinates.length >= 2) { // Need at least 2 points for a polyline
          _polylines.add(
            Polyline(
              polylineId: PolylineId(widget.route['id']?.toString() ?? 'default_route_${DateTime.now().millisecondsSinceEpoch}'), // Ensure unique ID
            ...
            debugPrint("Warning: Not enough points (${routeCoordinates.length}) to draw a polyline.");
        }
        // --- End Add Polyline ---
  ...
  LatLng _getLocationForStop(String stopName) {
    // (Your _getLocationForStop implementation remains the same)
     switch (stopName) {
      ...
      case 'UTAR Block D':
        return const LatLng(4.338115104744316, 101.1438004149668); // Near Faculty of Engineering 4.338115104744316, 101.1438004149668
      case 'UTAR Block G':
        return const LatLng(4.340407678390168, 101.14358161247885); // Near Faculty of Science 4.340407678390168, 101.14358161247885
      case 'UTAR Block N':
        return const LatLng(4.338909947691619, 101.13661859791222); // Near Faculty of Arts/Social Science 4.338909947691619, 101.13661859791222

      // Housing Areas (Kampar)
      case 'Stanford Bus Stop':
        return const LatLng(4.326598839352191, 101.13491463433371); // Near Stanford Apartment entrance 4.326598839352191, 101.13491463433371
      case 'V2 Optical Shop':
        return const LatLng(4.3262801748307735, 101.14343490343325); // Exact shop location 4.3262801748307735, 101.14343490343325
      case 'Taman Mahsuri Impian':
        return const LatLng(4.328170722263601, 101.1511850798773); // Main entrance bus stop 4.328170722263601, 101.1511850798773
      case 'Harvard & Cambridge':
        return const LatLng(4.3308804452733725, 101.13231301152658); // Between the two residences 4.3308804452733725, 101.13231301152658
      case 'Westlake 1':
        return const LatLng( 4.329125938362558, 101.13656163046677 ); // Westlake main bus stop got 3: 4.329125938362558, 101.13656163046677 / 4.329897666191284, 101.13918033684243 / 4.33078070699843, 101.13639460717039
       case 'Westlake 2':
      ...
      case 'Champs Elysees/The Trails':
        return const LatLng(4.319810902677306, 101.12434723845445); // Near security post 4.319810902677306, 101.12434723845445
      ...
      default:
        ...
        return const LatLng(4.3328, 101.1236); // Default to UTAR main entrance (or maybe Kampar town center?)
    }
  }
  ...
  Widget build(BuildContext context) {
     ...
     if (trips is List && trips.isNotEmpty && _selectedTripIndex < trips.length) {
        final tripData = trips[_selectedTripIndex];
        if (tripData is Map<String, dynamic>) {
           selectedTrip = tripData;
        } else {
          debugPrint("Error: Selected trip data at index $_selectedTripIndex is not a Map.");
        }
     } else {
        debugPrint("Error: Invalid 'trips' data or selected index $_selectedTripIndex out of bounds.");
     }
     // --- End Safe Access ---
     ...
     if (selectedTrip == null) {
        return Scaffold(
           appBar: AppBar(title: Text(widget.route['name'] ?? 'Route Details')),
           body: const Center(
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.route['name'] ?? 'Unknown Route'), // Use null check
      ),
      body: Column(
        children: [
          // Map section
          Expanded(
            ...
            child: GoogleMap(
              onMapCreated: (GoogleMapController controller) {
                ...
                // Call _showRouteOnMap AFTER the controller is assigned and map is likely ready
                // Delay ensures the map is fully rendered for animations
                 if (mounted) { // Check if widget is still in the tree
                    _showRouteOnMap();
                 }
              },
              initialCameraPosition: const CameraPosition(
                target: LatLng(4.3331, 101.1345), // UTAR Kampar approx center
                zoom: 14.0, // Slightly zoomed out initially
              ),
              markers: _markers,
          ...
          Expanded(
            flex: 3, // Give details slightly more space perhaps
              ...
              child: Column(
                children: [
                  TabBar(
                    ...
                    labelColor: Theme.of(context).colorScheme.primary, // Use ColorScheme
  ...
  Widget _buildScheduleTab(Map<String, dynamic> selectedTrip) {
    ...
    final allTrips = (allTripsData is List) ? allTripsData.whereType<Map<String, dynamic>>().toList() : <Map<String, dynamic>>[];
    final departures = (departuresData is List) ? departuresData.whereType<Map<String, dynamic>>().toList() : <Map<String, dynamic>>[];
    final notes = (notesData is List) ? notesData.whereType<String>().toList() : <String>[];

    // Safely handle arrival data (addressing the type cast error)
    String? arrivalStop;
    String? arrivalTime;
    if (arrivalData is Map<String, dynamic>) {
      arrivalStop = arrivalData['stop']?.toString();
      arrivalTime = arrivalData['time']?.toString();
    } else if (arrivalData != null) {
        // Log if arrival data is present but not a map
        debugPrint("Warning: 'arrival' data is not a Map: ${arrivalData.runtimeType}");
    }
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
           if (allTrips.isEmpty)
             const Text('No trips available for this route.')
           else
              SizedBox(
                height: 50, // Fixed height for chips
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final tripNumber = trip['tripNumber']?.toString() ?? 'Trip ${index + 1}';
                    return Padding(
                      padding: const EdgeInsets.only(right: 8),
                      child: ChoiceChip(
                        label: Text(tripNumber),
                        selected: _selectedTripIndex == index,
                        // Update state and redraw the map/details for the new trip
                        onSelected: (selected) {
                           if (selected) { // Only update if selected, not deselected
                              setState(() {
                                 _selectedTripIndex = index;
                                 // Re-calculate and display route for the newly selected trip
                                 _showRouteOnMap();
                              });
                           }
                        }
          ...
          Text('Blocks: ${selectedTrip['blocks']?.toString() ?? 'N/A'}',
              style: const TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 16), // More spacing
          const Text('Departure Times:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
           if (departures.isEmpty)
             ...
             ...departures.map<Widget>((departure) {
                ...
                return ListTile(
                  leading: const Icon(Icons.departure_board), // More relevant icon
                  title: Text(stopName),
                  trailing: Text(time),
          ...
          const SizedBox(height: 16), // More spacing
          // Display arrival info safely
          if (arrivalStop != null && arrivalTime != null)
            Text('Arrival at $arrivalStop: $arrivalTime',
                style: const TextStyle(fontWeight: FontWeight.bold))
          else
            const Text('Arrival information unavailable',
                style: TextStyle(fontStyle: FontStyle.italic)), // Indicate missing info

          // Display special note if present
          if (selectedTrip['specialNote'] != null && selectedTrip['specialNote'].toString().isNotEmpty)
            Padding(
              padding: const EdgeInsets.only(top: 12),
              child: Text('Note: ${selectedTrip['specialNote']}',
                  style: TextStyle(color: Colors.orange[800], fontStyle: FontStyle.italic)), // Slightly darker orange
            ),

          // Display route notes if present
          if (notes.isNotEmpty) ...[
            const SizedBox(height: 16),
            const Text('Route Notes:', style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  Widget _buildStopsTab() {
    ...
    final stopsToShow = (stopsData is List) ? stopsData.whereType<String>().toList() : <String>[];
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            title: Text(stop),
            subtitle: isFirst
                ? const Text('Starting Point', style: TextStyle(color: Colors.green))
                : isLast
                    ? const Text('Terminal Point', style: TextStyle(color: Colors.red))
                    : null,
            trailing: IconButton(
              icon: Icon(Icons.location_on, color: Theme.of(context).colorScheme.primary), // Theme color
              ...
              onPressed: () {
                ...
                if (controller != null) {
                  controller.animateCamera(
                    CameraUpdate.newLatLngZoom(_getLocationForStop(stop), 16.5), // Zoom in a bit more
                  );
                } else {
                   // Optionally show a message if map isn't ready
                   ScaffoldMessenger.of(context).showSnackBar(
                     const SnackBar(content: Text('Map is initializing...'), duration: Duration(seconds: 1)),
                   );
                }
              },
            ),
